...
export const useVacationStore = defineStore('vacation', {
  state: () => ({
    ...
    allowUpdate: (state) => {
      return state.data.vacationDate !== null && state.data.substituteUser && state.data.approverUsers.length === state.approversCount
    }, backupApprovers: (state) => {
      return state.data.approverUsers.map(({ id, user: { id: backupApproverId } }) => ({ id, backupApproverId }))
    ...
    patchData() {
      ...
      dataRequest.patchData(newData).then(response => {
      ...
      this.data.approverUsers = this.data.approverUsers.filter((approver: BackupApprover) => {
